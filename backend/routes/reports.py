...
def make_router(db, require_admin, get_current_user, compute_hours_report, enrich_leaves) -> APIRouter:
    ...
    async def export_calendar_grid(
        ...
        headers = ["#", "Name", "Rank", "Category", *day_headers,
                   "Present", "Absent", "Leave", "Tour", "OT (h)"]
        ...
        # A3 landscape ~397mm usable. #-col + Name+Rank+Cat + 31 days + 5 totals (P/AB/LV/TR/OT).
        col_widths_mm = [8, 24, 14, 12] + [9] * len(days) + [12, 12, 12, 12, 14]
        from reportlab.lib.pagesizes import A3
        pdf = _pdf_from_table(
            "Calendar Grid",
            headers, table,
