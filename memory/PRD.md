...
- **PayCraft `/leave-balances` key feed** now returns a NEW `lop_ytd_days` field per member (sum of `lop_days` on approved leaves for the year) so payroll can deduct LOP without reading the grid. Existing `balances` shape untouched — safe contract addition. Verified via curl (51 rows, real LOP values e.g. 40.0). PayCraft's automated pulls are: `/members` (roster), `/leave-balances` (leave + comp-off + now lop_ytd_days), `/dar/report` (DAR misses).
