        ...
        <div className="overflow-auto" style={{ maxHeight: "calc(100vh - 340px)" }}>
          <table className="text-xs border-collapse" data-testid="calendar-grid-table">
            <thead className="sticky top-0 z-30 bg-slate-50">
              <tr>
                {/* Sticky-left column widths MUST stay pinned exactly
                    to the values baked into the `left-*` offsets on
                    Member (40px) and Category (220px) — otherwise the
                    day-01 column gets tucked *under* Category and the
                    admin sees its left edge chopped when the row-set
                    changes serial-number width (bug seen when swapping
                    to the Staff & Coaches filter → 44 rows shrinks the
                    # column from 40px to ~30px). Locking min+max keeps
                    the offsets valid across every filter combo. */}
                ...
                <th className="py-1.5 px-2 text-left sticky left-[220px] z-40 bg-slate-50 w-[100px] min-w-[100px] max-w-[100px] hidden md:table-cell border-b border-slate-200 font-semibold text-slate-700">Category</th>
                {dayHeaders.map((h) => (
                  <th
                    key={h.iso}
              ...
              <tr>
                ...
                <th className="py-0.5 px-2 sticky left-[220px] z-40 bg-slate-50 w-[100px] min-w-[100px] max-w-[100px] hidden md:table-cell border-b border-slate-200"></th>
                {dayHeaders.map((h) => (
                  <th
                    key={h.iso + "-dow"}
              ...
              {displayedRows.map((r, i) => {
                ...
                return (
                  <tr key={r.member_id} className={`${rowBg} hover:bg-sky-50 group transition-colors`} data-testid={`calendar-row-${r.member_id}`}>
                    ...
                    <td className={`py-1 px-2 text-slate-600 hidden md:table-cell border-b border-slate-100 sticky left-[220px] z-20 w-[100px] min-w-[100px] max-w-[100px] ${rowBg} group-hover:bg-sky-50`}>{categoryLabel(r.category)}</td>
