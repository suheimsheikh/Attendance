...
### Twilio SMS — parent notifications wired (June 21 2026)
- `POST /api/parent-notify/dispatch` now SENDS a real SMS via Twilio (was record-only/device-composer). Helpers: `_e164_india()` (+91 normalise), `_twilio_send()` (blocking, via `asyncio.to_thread`), `_send_parent_sms()` (de-dupes father/mother/guardian numbers, per-number result). Endpoint composes a default message but prefers the client-supplied `message` (frontend `NotifyParentsButton` still builds the bilingual EN+Telugu body with callback numbers). Records dispatch only after >=1 SMS succeeds; graceful 400 when Twilio unconfigured or no parent number on file; 502 if all sends fail. Frontend no longer opens the device SMS composer (avoids double-send).
- Env: added empty TWILIO_ACCOUNT_SID / TWILIO_AUTH_TOKEN / TWILIO_FROM_NUMBER to backend/.env (USER TO FILL). `twilio==9.10.9` in requirements.txt.
- Mode: one-tap "Notify parents" button on Presence (per user choice c). Fully-automatic send on threshold = future task.
- Verified (no creds): 400 "SMS not configured"; no-contact member: 400; no false dispatch recorded. PENDING: real send test once user provides Twilio credentials + a verified test number.

### Per-institution Twilio senders + institution data recovery (June 21 2026)
- **Per-institution Twilio:** separate Twilio account (SID+token+from-number) per institution, stored in new `sms_senders` collection (keyed by institution name; `__default__` = fallback). Endpoints `GET /api/admin/sms-senders` (auth token masked, never returned), `PUT /api/admin/sms-senders/{institution}` (blank token keeps existing). Index on institution unique. Dispatch resolves sender via member.institution -> _sender_for_institution() (institution match else __default__); 400 if none configured. `_twilio_send` uses per-sender creds + 15s TwilioHttpClient timeout. Frontend: `pages/admin/SmsSenders.jsx` rendered at bottom of Office Settings — one row per institution (Default + each), masked token, per-row Save. No env fallback (per user). Verified: save/mask/keep-token, sender resolution reaches Twilio (502 on dummy creds, not 400), no-contact -> 400.
- ⚠️ **Institution data incident + recovery:** a normalization script had a duplicate-dict-key bug that overwrote ALL members'  institution to "YCH". Detected immediately and RECOVERED by re-reading the Sailors xlsx and restoring each sailor's institution (mapped to canonical institutions-collection names: RAINBOW HOME->Rainbow Home, AGAPE HOME->Agape Home). Final: YCH 92, MJPT 19, Rainbow Home 12, Agape Home 13. Integrity confirmed: 94 athletes w/ DOB, 83 w/ parent contact (= sheet minus 13 Agape no-contact). Non-sailor staff/coaches default to YCH (acceptable; original null/other values for a handful not separately recoverable). LESSON: never build a Mongo filter with a duplicated key.

### Prerecorded parent voice call — Telugu + Hindi (June 21 2026)
- Voice call now accompanies the parent SMS (one-tap "Notify parents" does SMS + call together). Backend: `voice_messages` collection stores one clip per type (late/absent) as base64. `POST /api/admin/voice-messages/{type}/generate` synthesises Telugu+Hindi via OpenAI TTS (emergentintegrations `OpenAITextToSpeech`, tts-1, voice alloy, Emergent LLM key); `POST .../upload` replaces with an admin mp3 (<=5MB); PUBLIC `GET /api/voice-messages/{type}/audio` serves audio/mpeg for Twilio `<Play>`. Dispatch builds the public audio URL from request proto+host and places best-effort Twilio voice calls (`client.calls.create(twiml=<Response><Play>url</Play></Response>)`) to each parent number using the institution sender; call failure does NOT fail the SMS. Records channel twilio_sms+voice and calls_placed.
- Frontend: `pages/admin/VoiceMessages.jsx` in Office Settings — per type: status, audio preview, Generate (TE+HI), Upload mp3. Verified: both clips generated (late 9s, absent 13s mp3), public serve returns audio/mpeg 200, UI players work.
- ⚠️ Real CALL needs a VOICE-CAPABLE Twilio number on the institution sender + live creds (untested end-to-end like SMS). TTS quality acceptable for now; user to record/upload real voices later.
